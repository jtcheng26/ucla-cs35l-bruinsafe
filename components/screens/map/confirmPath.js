...
export default function confirmPath({
  onPress,
  coordinates,
  setMarkerList,
  setThePath,
  ...
  const handleClick = async (confirm) => {
    if (confirm) {
      setWalking(true);
      setWaiting(true);
      const pushCoords = await axios.post(BASE_URL + "/walk/request", {
        origin: copyPath.start,
        destination: copyPath.end,
        user: id,
        timeLeft: timeLeft,
      }); //Create walk request
      setConfirmed(true);
  ...
  return (
    <View className="absolute bottom-32 content-center space-x-5 w-full flex-row items-center align-center justify-center">
      <TouchableOpacity
        className="bg-blue-500/90 py-3 rounded-2xl w-5/12"
        onPress={() => handleClick(true)}
      >
        <Text className="flex justify-center text-zinc-200 text-base font-bold text-center">
          Confirm
        </Text>
      </TouchableOpacity>
      <TouchableOpacity
        className="bg-zinc-200/70 py-3 rounded-2xl w-5/12"
