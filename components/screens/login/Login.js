...
export default function Login() {
  const styles = {
    inputField:
      "w-full h-14 text-blue-200 rounded-full text-md bg-blue-700 px-6 my-4 text-justify",
    inputBox:
      "text-blue-200 pb-2 pt-1 w-full text-lg mb-4 justify-start border-b-2 border-blue-200",
  ...
  const quickCheck = (email, pw) => {
    ...
    if (!isPwValid(pw)) {
      if (pw.length() < 8) setLogError("Must have minimum of 8 characters");
      else
        setLogError(
      ...
      const sendUser = async () => {
        try {
          ...
          await AsyncStorage.setItem("@name", response.data.name)
            .then(() => console.log("Name saved ", response.data.name))
            .catch((error) => console.log("Error saving data: ", error));
          await AsyncStorage.setItem("@id", response.data._id)
            .then(() => console.log("ID saved ", response.data._id))
            .catch((error) => console.log("Error saving data: ", error));
          await AsyncStorage.setItem("@pw", pw)
            .then(() => console.log("PW saved ", pw))
            .catch((error) => console.log("Error saving data: ", error));
          setPW(null);
          setEmail(null);
          setConfirmPW(null);
  ...
  if (signupSuccess) {
    //on sign in/log in send to main page
    return <MainPage onLogout={setSignupSuccess} />;
  }

  return (
    <View className="w-full h-full bg-blue-900/90 items-center justify-center pt-20">
      <Text className="text-5xl text-amber-400 font-extrabold mb-10">
        BruinSafe
      </Text>
      <View className="w-10/12">
        <Text className={styles.inputText}>Email:</Text>
        <TextInput
        ...
        <Text className={styles.inputText}>Password:</Text>
        <TextInput
          secureTextEntry
          className={styles.inputBox}
          placeholderTextColor={"rgb(2 132 199)"}
        ...
        {suTxt == "SIGN UP" ? (
          <>
            <Text className={styles.inputText}>Confirm Password:</Text>
            <TextInput
              secureTextEntry
              className={styles.inputBox}
              placeholder=""
      ...
      >
        <Text className="text-blue-900/90 text-md font-semibold">{suTxt}</Text>
      </TouchableScale>
      ...
      <TouchableScale
        ...
        activeScale={0.97}
      >
        <Text className="text-blue-300 text-md font-semibold">
          {suTxt == "SIGN UP" ? "BACK" : "NEW USER?"}
        </Text>
      </TouchableScale>

      <Text className="my-6 text-lg font-semibold text-red-500">
        {logError}
      </Text>
    </View>
  );
